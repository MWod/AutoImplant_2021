...
Continuation of training for combined Task 1/3 (using registration) - for HPC.
"""
...
def experiment(current_run):
    ...
    experiment_name = "Combined_HPC_Exp1"
    experiments_family = "Combined_HPC_Experiments"
    ...
    transforms = aug.generate_transforms_flip_affine(scales=(0.85, 1.15), degrees=(-10, 10), translation=(-9, 9))
    training_mode = "defect_implant"
    checkpoints_path = p.checkpoints_path / experiments_family / experiment_name
    to_load_checkpoint = p.checkpoints_path / experiments_family / "Combined_HPC_Exp1" / "cp3" if int(current_run) <= 1 else p.checkpoints_path / experiments_family / experiment_name / str("cp" + str(int(current_run) - 1))
    ...
    model_save_path = p.combined_hpc_exp1_save_path / str("model_cp" + current_run)

    log_dir = p.logs_path / experiments_family / experiment_name / str("cp_" + current_run)
    comment = "Continuation of training for combined Task 1/3 (using registration) - HPC"
