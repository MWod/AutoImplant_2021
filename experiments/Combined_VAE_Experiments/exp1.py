...
Training for combined Task 1/3 (using registration and VAE-based generation) (with only geometric augmentation - but stronger).
"""
...
def experiment(current_run):
    ...
    experiment_name = "Combined_VAE_Exp1"
    experiments_family = "Combined_VAE_Experiments"
    ...
    # transforms = aug.generate_transforms_flip_affine(scales=(0.85, 1.15), degrees=(-15, 15), translation=(-10, 10))
    ...
    data_path = p.combined_vae_training_path
    training_csv = p.combined_vae_training_csv_path
    validation_csv = p.combined_vae_validation_csv_path
    model_save_path = p.combined_vae_exp1_save_path / str("model_cp" + current_run)

    log_dir = p.logs_path / experiments_family / experiment_name / str("cp_" + current_run)
    comment = "Training for combined Task 1/3 (using registration and VAE-based generation) (with only geometric augmentation - but stronger)."
