...
Continuation of training for combined Task 1/3 (using registration) (with only geometric augmentation - but stronger).
"""
...
def experiment(current_run):
    ...
    experiment_name = "Combined_Exp3"
    experiments_family = "Combined_Experiments"
    ...
    transforms = aug.generate_transforms_flip_affine(scales=(0.92, 1.08), degrees=(-9, 9), translation=(-9, 9))
    training_mode = "defect_implant"
    checkpoints_path = p.checkpoints_path / experiments_family / experiment_name
    to_load_checkpoint = p.checkpoints_path / experiments_family / "Combined_Exp1" / "cp3" if int(current_run) <= 1 else p.checkpoints_path / experiments_family / experiment_name / str("cp" + str(int(current_run) - 1))
    ...
    training_csv = p.combined_training_csv_path
    validation_csv = p.combined_validation_csv_path
    model_save_path = p.combined_exp3_save_path / str("model_cp" + current_run)

    log_dir = p.logs_path / experiments_family / experiment_name / str("cp_" + current_run)
    comment = "Continuation of training for combined Task 1/3 (using registration) (with only geometric augmentation - but stronger)."
