...
Training for combined Task 1/3 (without registration) (with only geometric augmentation).
"""
...
def experiment(current_run):
    ...
    training_csv = p.combined_training_wr_csv_path
    validation_csv = p.combined_validation_csv_path
    model_save_path = p.combined_exp2_save_path / str("model_cp" + current_run)

    log_dir = p.logs_path / experiments_family / experiment_name / str("cp_" + current_run)
    comment = "Training for combined Task 1/3 (without registration) (with only geometric augmentation)."
